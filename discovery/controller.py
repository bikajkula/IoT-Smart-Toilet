...
cred_object = firebase_admin.credentials.Certificate('iot-smart-wc-solja-firebase-adminsdk-1hj2h-c98a10e515.json')
default_app = firebase_admin.initialize_app(cred_object, {
	'databaseURL': "https://iot-smart-wc-solja-default-rtdb.firebaseio.com/"
	})
	
ref = db.reference("/broker/")
json_string_fire = '{"broker_ip": "' + get_local_non_loopback_address() + '"}'
file_contents = json.loads(json_string_fire)
	...
	while True:
		try:
			...
			if "byebye" in msg:
				...
				ref = db.reference("/devices/"+configId)
				ref.set(json_data)
